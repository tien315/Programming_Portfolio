...
types=["glass", "wood", "computer", "plastic", "styrofoam", "wood", ...
    "battery", "cartridge"];
data = [mindist;maxdist;avdist;time;ghg];

x_axis = [1:size(types,2)];

subplot(3,1,1);
hold all;
grid on;
plot(x_axis, data(1,1:end), 'r--o', 'LineWidth', 2, 'MarkerSize', 6);
plot(x_axis, data(2,1:end), 'b--s', 'LineWidth', 2, 'MarkerSize', 6);
plot(x_axis, data(3,1:end), 'g--d', 'LineWidth', 2, 'MarkerSize', 6);
...
hold all;
grid on;
plot(x_axis, data(5,1:end), 'sk', 'MarkerSize', 10, 'MarkerFaceColor',...
    'black');
plot(find(data(5,1:end) > 0.05), data(5,(data(5,1:end) > 0.05)),...
    'or', 'MarkerSize', 25, 'LineWidth', 2);
...
hold all;
grid on;
plot(x_axis, data(4,1:end), '*g', 'MarkerSize', 20);
plot(find(data(4,1:end) > 400), data(4, (data(4,1:end) > 400)),...
    'or', 'Markersize', 25, 'LineWidth', 2);
